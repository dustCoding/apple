...
# 1차원 배열 입력받기
# map함수를 이용하면 for문을 쓰지 않고 한줄에 여러개의 정수를 입력받을 수 있다.
# 기준 : 공백
# num_list = list(map(int, input().split()))

num = int(input())
fileNames = [input() for _ in range(num)]
# print(fileNames)
lenN = len(fileNames[0])
arr = [0 for i in range(lenN)]
# for i in fileNames:
#     print("list : ",list(i))
#     print("-----------------")
for i in range(0,num):
    for j in range(0,lenN): 
        if i<num-1:
            # print("len:",lenN)
            # print("j:",j)
            # print("fileNames i:",i," ",j," = ",fileNames[i][j])
            # print("fileNames i+1:",i," ",j," = ",fileNames[i+1][j])
            if fileNames[i][j]==fileNames[i+1][j]:
                arr[j]=fileNames[i][j]
            else:
                arr[j]="?"
        if i==num-1:
            # print("j==lenN-1 len:",lenN)
            # print("j==lenN-1 j:",j)
            # print("fileNames j:",i," ",j," = ",fileNames[i][j])
            # print("fileNames 0:",i," ",j," = ",fileNames[0][j])
            if arr[j]!=fileNames[0][j]:
                arr[j]="?"
